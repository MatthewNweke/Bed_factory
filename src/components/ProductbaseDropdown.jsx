...
const ProductbaseDropdown = () => {
  const [activeItem, setActiveItem] = useState("Dimensions"); // Initialize activeItem with 'Dimensions'
  ...
  const dropdownContent = (itemName) => {
    switch (itemName) {
      case "Dimensions":
        return (
          <div className="">
      ...
      <div className=" h-24 relative w-[100%]">
        <ul className="p-0 m-0 flex items-center justify-center h-[100%]  gap-5 max-sm:gap-2">
          {[
            ...
            <li
              key={item}
              className={`cursor-pointer px-6 max-sm:px-4 py-2 text-sm rounded-xl ${
                activeItem === item ? "border-[1px] border-black" : ""
              }`}
              onClick={() => handleItemClick(item)}
            >
              {item}
              {activeItem === item && (
                <div className="absolute  top-full left-0 bg-white w-[100%] border border-gray-200 mt-1 animate__animated animate__fadeIn rounded shadow-lg">
